...
async function promptGitHubTokenAndId() {
  let token = localStorage.getItem("github_token") || prompt("Enter your GitHub Personal Access Token (with gist/repo access):");
  if (!token) return {};

  localStorage.setItem("github_token", token);

  // Ask for gist/repo ID or offer to create
  let fileId = localStorage.getItem("github_file_id") || prompt("Enter your GitHub Gist ID (or repo:owner/repo/branch/file.json):");
  if (!fileId) return {};

  localStorage.setItem("github_file_id", fileId);

  return { token, fileId };
}

// --- SYNC from GitHub Gist or File ---
async function githubSync() {
  const { token, fileId } = await promptGitHubTokenAndId();
  if (!token || !fileId) return alert("Sync cancelled.");

  if (fileId.includes('/')) {
    // Looks like repo path (owner/repo/branch/file)
    const [owner, repo, branch, ...fileArr] = fileId.split('/');
    const filePath = fileArr.join('/');
    const url = `https://api.github.com/repos/${owner}/${repo}/contents/${filePath}?ref=${branch}`;
    const resp = await fetch(url, {
      headers: { Authorization: `token ${token}` }
    });
    if (!resp.ok) return alert("GitHub sync failed (repo): " + (await resp.text()));
    ...
    try {
      ...
      if (Array.isArray(imported) && imported.every(x => typeof x === "object" && x.type)) {
        ...
        save();
        render();
        alert("Synced from GitHub successfully!");
      } else alert("Invalid format on GitHub");
    } catch (e) {
      alert("Could not parse data from GitHub");
    }
    return;
  }

  // Otherwise, treat as Gist
  const url = `https://api.github.com/gists/${fileId}`;
  const resp = await fetch(url, {
    headers: { Authorization: `token ${token}` }
  });
  if (!resp.ok) return alert("GitHub sync failed (gist): " + (await resp.text()));
...
// --- SAVE to GitHub Gist or File ---
async function githubSave() {
  const { token, fileId } = await promptGitHubTokenAndId();
  if (!token || !fileId) return alert("Save cancelled.");
  ...
  if (fileId.includes('/')) {
    ...
    const saveResp = await fetch(url, {
      ...
      body: JSON.stringify({
        message: "Update trackers data",
        content: btoa(unescape(encodeURIComponent(JSON.stringify(data, null, 2)))),
        sha,
        branch,
      })
    });
    if (!saveResp.ok) return alert("GitHub save failed (repo): " + (await saveResp.text()));
    alert("Saved to GitHub successfully!");
    return;
  }

  // Otherwise, treat as Gist
  // We'll use the first file in the gist
  const gistUrl = `https://api.github.com/gists/${fileId}`;
  const resp = await fetch(gistUrl, {
    headers: { Authorization: `token ${token}` }
  });
  if (!resp.ok) return alert("GitHub save failed (gist): " + (await resp.text()));
  const gist = await resp.json();
  const fileName = Object.keys(gist.files)[0];
  const patchResp = await fetch(gistUrl, {
    method: "PATCH",
    headers: { "Authorization": `token ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({
      files: {
  ...
  if (!patchResp.ok) return alert("GitHub save failed (gist): " + (await patchResp.text()));
  alert("Saved to GitHub successfully!");
}

// =============== HOOK UP BUTTONS ===================

document.getElementById("github-sync").onclick = githubSync;
document.getElementById("github-save").onclick = githubSave;

// =================== END GITHUB SYNC CODE ===================

...
function render() {
  ...
  // Render top-level trackers
  const rootTrackers = data.filter(i => i.type === "tracker" && i.name.toLowerCase().includes(filter));
  ...
  data.filter(i => i.type === "folder").forEach(folder => {
    if (!folder.name.toLowerCase().includes(filter) && folder.trackers.every(t => !t.name.toLowerCase().includes(filter))) return;
...
function createTrackerCard(tracker, parentFolder = null) {
  ...
  el.innerHTML = `
    ...
    <div>
      <button onclick="openTransactions('${tracker.id}', ${!!parentFolder})">🧾</button>
...
function createFolderCard(folder, filter) {
  ...
  header.innerHTML = `
    <span>${isExpanded ? "▼" : "▶️"} <strong>${folder.name}</strong> (${folder.trackers.length})</span>
    <span>
...
function initializeDragAndDrop() {
  ...
  data.filter(i => i.type === "folder").forEach(folder => {
    const listEl = [...document.getElementsByClassName("folder")].find(f => f.querySelector(".folder-header strong").textContent === folder.name);
    if (!listEl) return;
...
// Hook buttons to modal functions
